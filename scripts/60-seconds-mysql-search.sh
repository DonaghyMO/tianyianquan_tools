...
# -*- coding: utf-8 -*-
MYSQL_PWD=1998
TMP_PATH='/usr/local/go/bin:/root/go/bin:/usr/local/go/bin:/root/go/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin'
    ...
    for row in "${commands_array[@]}"; do
        ...
        # 在子进程中执行命令
        {
            ...
            if [ $exit_code -eq 124 ]; then
                data="Command execute failed"
                # 更新状态为20050，并将data存入数据库，假设$id为脚本中定义的某个变量
                update_query="UPDATE tbl_result t SET t.status=20051, t.data=\"$data\" WHERE id=$id"
            else
                # 对data中的特殊字符进行转义
